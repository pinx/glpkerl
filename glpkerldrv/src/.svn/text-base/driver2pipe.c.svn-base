...
void
set_port_control_flags (ErlDrvPort port,
                        int        flags)
{
  (void) port;
  (void) flags;
...
{
  return driver_alloc (sizeof (ErlDrvBinary) + size);
}

void
driver_free_binary (ErlDrvBinary* b)
{
  free (b);
}
...
// limitations:
// 1. we can only send singleton binary terms,
// 2. we ignore the 'to' argument and end up sending to the port owner

int 
driver_send_term (ErlDrvPort        ix, 
                  ErlDrvTermData    to,
                  ErlDrvTermData*   data, 
                  int               len)
{
  ...
  if (len != 4 || data[0] != ERL_DRV_BINARY)
    {
      return -1;
    }

  bin = (ErlDrvBinary*) data[1];

  return driver_output (ix, bin->orig_bytes + data[3], data[2]);
}
      ...
      if (info.fds[0].revents)
        {
          ...
          rlen = entry->control (d, buf[0], buf + 1, size - 1, &rbuf, 0);

          if (rbuf)
            {
              write_packet (4, 4, ((ErlDrvBinary*) rbuf)->orig_bytes, rlen);
              driver_free_binary ((ErlDrvBinary*) rbuf);
            }
          else
            {
              write_packet (4, 4, NULL, 0);
            }
        }
